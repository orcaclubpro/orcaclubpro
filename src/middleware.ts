...
import { GONE_PATHS } from '@/lib/seo/redirect-map.mjs'
...
export function middleware(request: NextRequest) {
  ...
  // Retired pages with no topical successor: honest 410 (see redirect-map.mjs —
  // next.config redirects() cannot emit 410, so these live here).
  ...
  // SONAR lives at orcaclub.pro/sonar since the Phase-2 cutover. The old
  // subdomain (sonar.orcaclub.pro) and internal /s mount 301 to it via
  // next.config.mjs redirects() — no host-based routing left here.
