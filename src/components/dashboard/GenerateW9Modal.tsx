...
import { Check, Download, Loader2, Mail, ScrollText, ShieldAlert } from 'lucide-react'
...
// PDF: no draft is saved, no document is filed, and the fields are cleared when
// the dialog closes. That is the deliberate trade — a few seconds of typing in
// exchange for a TIN that is never at rest anywhere in the system.
...
export interface GenerateW9ModalProps {
  open: boolean
  onClose: () => void
...
function requesterBlock(props: GenerateW9ModalProps): string {
...
export function GenerateW9Modal(props: GenerateW9ModalProps) {
  const { open, onClose, recipients = [], accountReference, signerName } = props
  ...
  /** Wipe the form — in particular the TIN — whenever the dialog is dismissed. */
  const close = () => {
    setTin('')
    setOutcome(null)
    onClose()
  ...
  return (
    <Dialog open={open} onOpenChange={o => { if (!o) close() }}>
      <DialogContent className="max-w-2xl max-h-[85vh] overflow-y-auto bg-[var(--space-bg-card)] border-[var(--space-border-hard)]">
        <DialogTitle className="flex items-center gap-2 text-[var(--space-text-primary)]">
        ...
        <div className="space-y-3 border-t border-[var(--space-border-hard)] pt-4">
          ...
          <div className="flex flex-wrap items-center justify-end gap-2">
            <button
              type="button"
              onClick={close}
              className="px-3 py-2 text-xs text-[var(--space-text-muted)] transition-colors hover:text-[var(--space-text-tertiary)]"
            >
              Close
            </button>
            <button
