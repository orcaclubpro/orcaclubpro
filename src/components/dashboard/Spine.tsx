...
export function Spine({
  ...
  if (months.length === 0) {
    return (
      <p className="rounded-xl border border-dashed border-[var(--space-border-hard)] px-5 py-8 text-center text-[13px] text-[var(--space-text-muted)]">
        {emptyMessage}
      </p>
    )
  }
        ...
        {months.map((month) => (
          <Fragment key={month.key}>
            <li className="relative flex items-center gap-3 py-2">
              ...
              <span className="ml-2 text-[11px] font-medium uppercase tracking-[0.18em] text-[var(--space-text-muted)]">
                {month.label}
              </span>
            </li>
...
function TodayRule() {
  return (
    <li className="relative flex items-center gap-3 py-1.5" aria-label="Today">
      <span className="w-14 shrink-0 text-right text-[11px] font-medium text-[var(--space-accent)]">
        Today
      </span>
  ...
  const body = (
    <>
      <time
        ...
        className="w-14 shrink-0 pt-px text-right text-[11px] tabular-nums text-[var(--space-text-muted)]"
      >
      ...
      <span className="ml-2 min-w-0 flex-1">
        <span className="flex items-baseline gap-2">
          <span className="truncate text-[13px] font-medium text-[var(--space-text-primary)]">
            {event.title}
          </span>
          <span className="shrink-0 text-[11px] text-[var(--space-text-muted)]">
            {KIND_LABEL[event.kind]}
          </span>
        </span>
        {event.meta && (
          <span className="mt-0.5 block truncate text-[12px] text-[var(--space-text-secondary)]">
            {event.meta}
          </span>
        )}
      </span>
      ...
      {typeof event.amount === 'number' && (
        <span className="shrink-0 pt-px text-[12px] tabular-nums" style={{ color }}>
