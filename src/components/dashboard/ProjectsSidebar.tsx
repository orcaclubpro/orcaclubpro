...
function ProjectRow({
  ...
  return (
    <Link
      ...
      onClick={onClose}
      className="group flex items-center gap-3 px-4 py-3 rounded-xl hover:bg-white/[0.04] transition-colors duration-150"
    >
      {/* Status dot */}
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex items-center gap-2 mt-1">
          <div className="flex-1 h-1 bg-white/[0.06] rounded-full overflow-hidden">
            <div
              className="h-full bg-intelligence-cyan/50 rounded-full transition-all duration-500"
              style={{ width: `${progress}%` }}
            />
          </div>
          <span className="text-[9px] text-gray-600 tabular-nums shrink-0 w-6 text-right">
            {progress}%
          </span>
        </div>
        <p className="text-[9px] text-gray-600 mt-0.5 uppercase tracking-wider">{label}</p>
      </div>

      {/* Arrow */}
      <ArrowRight className="size-3 text-gray-700 group-hover:text-gray-400 group-hover:translate-x-0.5 transition-all duration-150 shrink-0" />
    </Link>
  )
}
  ...
  return (
    <div className="space-y-1">
      ...
      <div className="flex items-center gap-2 px-4 pb-3">
        <span className="inline-flex items-center gap-1 text-[9px] font-semibold uppercase tracking-widest px-2 py-0.5 rounded-full bg-intelligence-cyan/10 text-intelligence-cyan border border-intelligence-cyan/20">
          <span className="size-1 rounded-full bg-intelligence-cyan" />
          {activeCount} active
        </span>
        <span className="text-[9px] text-gray-600">{projects.length} total</span>
      </div>

      {sorted.length === 0 ? (
        <p className="text-xs text-gray-600 px-4 py-6 text-center">No projects yet.</p>
      ) : (
        sorted.map((project) => (
          <ProjectRow
            key={project.id}
...
export function ProjectsSidebar({
  ...
  return (
    <>
      ...
      >
        <Layers className="size-3.5 text-intelligence-cyan" />
        <span
      ...
      >
        <Layers className="size-4 text-intelligence-cyan" />
