  ...
  return (
    <div ref={containerRef} className="space-y-3">
      ...
      <div className="relative rounded-xl border border-[var(--space-border-hard)] bg-[var(--space-bg-card)] overflow-hidden">
        ...
        >
          <div style={{ width: leftW + timelineWidth, minWidth: '100%' }}>
            ...
            <div className="flex border-b border-[var(--space-border-hard)]" style={{ height: HDR_H }}>
              ...
              >
                <p className="text-[9px] tracking-[0.3em] uppercase text-[var(--space-text-secondary)]">{isMobile ? 'Proj' : 'Project'}</p>
              </div>
              ...
              return (
                ...
                >
                  ...
                  <div
                    ...
                    style={{ width: leftW, minWidth: leftW, gap: isMobile ? 6 : 10, paddingLeft: isMobile ? 8 : 16, paddingRight: isMobile ? 4 : 16 }}
                  >
                    <div
                    ...
                    <a
                      ...
                      className="truncate hover:text-[var(--space-text-primary)] transition-colors leading-snug font-medium flex-1 min-w-0"
                      style={{ fontSize: isMobile ? 10 : 11, color: '#A0A0A0' }}
                      title={project.name}
                    >
                      {project.name}
                    </a>
            ...
            {/* "N more" row */}
            {hiddenCount > 0 && (
              <div className="flex border-b border-[var(--space-border-hard)]" style={{ height: MORE_ROW_H }}>
                <div
                  className="sticky left-0 z-20 bg-[var(--space-bg-card)] border-r border-[var(--space-border-hard)] shrink-0 flex items-center"
                  style={{ width: leftW, minWidth: leftW, paddingLeft: isMobile ? 8 : 16 }}
                >
            ...
            {/* Orders row */}
            {allOrders.length > 0 && (
              <div className="flex border-t border-[var(--space-border-hard)]" style={{ height: ORDERS_ROW_H }}>
                <div
                  className="sticky left-0 z-20 bg-[var(--space-bg-card)] border-r border-[var(--space-border-hard)] shrink-0 flex flex-col justify-center gap-0.5"
                  style={{ width: leftW, minWidth: leftW, paddingLeft: isMobile ? 8 : 16, paddingRight: isMobile ? 4 : 16 }}
                >
                  <p className="text-[9px] tracking-[0.2em] uppercase text-[var(--space-text-secondary)]">Orders</p>
                  <p className="text-[8px] text-[var(--space-text-secondary)]">{allOrders.length} total</p>
                </div>
        ...
        {/* ── Legend ───────────────────────────────────────────────────────── */}
        <div className="px-5 py-3.5 border-t border-[var(--space-border-hard)] space-y-3">
          ...
          {projects.length > 1 && (
            <div className="flex items-center flex-wrap gap-x-5 gap-y-1.5 pt-2 border-t border-[var(--space-border-hard)]">
              {projects.map((p, i) => {
                const c = PROJECT_COLORS[i % PROJECT_COLORS.length]
                return (
                  <div key={p.id} className="flex items-center gap-1.5">
                    <div className="size-2.5 rounded-sm shrink-0" style={{ background: c.hex }} />
                    <span className="text-[9px] text-[var(--space-text-secondary)] truncate max-w-[100px]">{p.name}</span>
                  </div>
                )
              })}
            </div>
      ...
      <Dialog open={settingsOpen} onOpenChange={setSettingsOpen}>
        <DialogContent className="bg-[var(--space-bg-card)] border border-[var(--space-border-hard)] text-[var(--space-text-primary)] max-w-xs rounded-2xl">
          ...
          <div className="space-y-6 pt-1">
            <div>
              <p className="text-[10px] text-[var(--space-text-secondary)] uppercase tracking-[0.25em] mb-4">Projects shown</p>
              <div className="flex items-center gap-4">
                ...
                <span className="text-3xl font-bold tabular-nums text-[var(--space-text-primary)] w-8 text-center">{projectLimit}</span>
                <button
