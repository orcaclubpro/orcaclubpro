...
function Step2({
  data, onChange, onServiceSelect, onNext, onBack, services,
}: {
  data: FormData
...
function Step3({
  ...
  onBack, onSubmit, isSubmitting, showBookingOption,
}: {
  data: FormData
  mode: ConnectMode
  ...
  return (
    <div className="space-y-4">
      <div>
        ...
        <div className={`grid gap-3 ${showBookingOption ? 'grid-cols-2' : 'grid-cols-1'}`}>
          {showBookingOption && (
            <button
              type="button"
...
export default function ContactFormSection({
  ...
  tagline = 'New Project',
  contactEmail = 'chance@orcaclub.pro',
  contactLocation = 'California 714',
  contactPhone = 'By appointment',
  step1Title = 'Schedule a Call.',
  step1Sub = 'Schedule a free call to start your project.',
  step2Title = 'Your project.',
  step2Sub = 'What are you looking to build?',
  step3Title = 'How to connect.',
  ...
  const stepHeadings: Record<Step, { title: string; sub: string }> = {
    1: { title: step1Title, sub: step1Sub },
    2: { title: step2Title, sub: step2Sub },
    3: { title: step3Title, sub: step3Sub },
  }
  const headingData = stepHeadings[step]
  const resolvedServices = services.length > 0 ? services : DEFAULT_SERVICES
    ...
    <div className="relative z-10 flex h-screen">
      ...
      <div className="hidden lg:flex flex-col w-[50%] px-16 py-14">
        <div className="flex-1 flex items-center">
          <div>
            <h2 className={`${gothic.className} text-[4.5rem] text-white leading-[1.1]`}>
            ...
            <div className="mt-8 space-y-3">
              <div className="flex items-center gap-3">
                ...
                <p className="text-lg text-white/50 font-light">{contactEmail}</p>
              </div>
              <div className="flex items-center gap-3">
                <MapPin className="w-3.5 h-3.5 text-white/25 flex-shrink-0" />
                <p className="text-lg text-white/50 font-light">{contactLocation}</p>
              </div>
              <div className="flex items-center gap-3">
                <Phone className="w-3.5 h-3.5 text-white/25 flex-shrink-0" />
                <p className="text-lg text-white/50 font-light">{contactPhone}</p>
              </div>
            </div>
          </div>
        </div>
        <div className="flex items-center gap-4">
          ...
          <span className="text-[10px] tracking-[0.4em] uppercase text-white/25 font-light">{tagline}</span>
        </div>
      </div>
      ...
      >
        ...
        <div className="lg:hidden px-6 pt-8 pb-5 flex-shrink-0 border-b border-white/[0.07]">
          <h1 className={`${gothic.className} text-xl text-white mb-1`}>
            Start a conversation.
          </h1>
          <p className="text-xs text-white/35 font-light">{contactEmail} · {contactLocation}</p>
        </div>
          ...
          <div className="w-full max-w-[420px]">
            ...
            <div className="mb-6">
              ...
              <AnimatePresence mode="wait" custom={dir}>
                ...
                >
                  <h2 className="text-3xl font-light text-white tracking-wide">{headingData.title}</h2>
                  <p className="text-base text-white/40 mt-2 font-light">{headingData.sub}</p>
              ...
              <AnimatePresence mode="wait" custom={dir}>
                {step === 1 && (
                  <motion.div key="s1" custom={dir} variants={variants}
                    ...
                    <Step1 data={formData} onChange={handleChange} onNext={() => navigate(2)} />
                  </motion.div>
                )}
                {step === 2 && (
                ...
                {step === 3 && (
                  <motion.div key="s3" custom={dir} variants={variants}
                    ...
                    <Step3
                      ...
                      showBookingOption={showBookingOption}
                    />
                  </motion.div>
                )}
              </AnimatePresence>
