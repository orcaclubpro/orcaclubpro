...
export function ClientsView({ clientAccounts, username, userRole }: ClientsViewProps) {
        ...
        <div>
          <div className="flex items-baseline gap-3">
            <h1 className="text-3xl sm:text-4xl font-bold text-white tracking-tight">
              {userRole === 'admin' ? 'All Clients' : 'Your Clients'}
            </h1>
      ...
      {clientAccounts.length > 0 ? (
          ...
          {clientAccounts.map((client, index) => {
            ...
            return (
              ...
              >
                ...
                <div className={`relative z-[2] size-9 rounded-lg ${palette.bg} border ${palette.border} flex items-center justify-center shrink-0`}>
                  ...
                  {outstanding ? (
                    <>
                      ...
                      <div className="flex items-center justify-end gap-1.5 mt-0.5">
                        ...
                        <span className="text-[10px] text-amber-600 uppercase tracking-wider">due</span>
                      </div>
                    </>
                  ) : (
                    <div className="text-sm font-mono text-emerald-500/60 tabular-nums">$0.00</div>
                  )}
                </div>

                <div className="relative z-[2] w-20 text-center hidden sm:block shrink-0">
                  <div className="text-sm font-mono font-semibold text-white tabular-nums">{orders}</div>
        ...
        <div className="relative overflow-hidden rounded-xl border border-white/[0.08] bg-[#1c1c1c]">
          ...
          <div className="relative z-10 flex flex-col items-center text-center py-20 px-6">
            ...
            {userRole === 'admin' && <NewClientModal username={username} />}
          </div>
        </div>
      )}
    </div>
