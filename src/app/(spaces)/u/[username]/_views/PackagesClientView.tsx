            ...
            {addOns.length > 0 && (
              <div className="px-5 py-5 border-b border-[var(--space-border)]">
                <div className="flex items-center justify-between mb-1">
                  <span className="text-[0.5625rem] font-bold tracking-[0.25em] uppercase text-[var(--space-text-muted)]">Available Add-ons</span>
                  <span className="text-[0.5625rem] font-semibold tabular-nums text-[var(--space-text-muted)]">
                    {addOns.length} option{addOns.length === 1 ? '' : 's'}
                  </span>
                </div>
                <p className="text-[0.6875rem] text-[var(--space-text-muted)] leading-relaxed mb-3">
                  ...
                  {addOns.map((item, i) => {
                    ...
                    return (
                      <div key={i} className={cn('flex items-start gap-3 py-3', !isLast && 'border-b border-[var(--space-border)]')}>
                        ...
                        <div className="flex-1 min-w-0">
                          <p className="text-[0.8125rem] font-medium text-[var(--space-text-secondary)] leading-snug">
                            {item.name}
                            {qty > 1 && <span className="text-[var(--space-text-muted)]"> &times; {qty}</span>}
                          </p>
                          {item.description && (
                            <p className="text-[0.6875rem] text-[var(--space-text-muted)] mt-0.5 leading-relaxed whitespace-pre-line">{item.description}</p>
                          )}
                        </div>
                        <span className="shrink-0 pl-2 text-[0.8125rem] font-semibold tabular-nums text-[var(--space-text-muted)]">
                          {fmt(unit * qty)}
                          {item.isRecurring && (
                            <span className="text-[0.6875rem] font-normal">/{item.recurringInterval === 'year' ? 'yr' : 'mo'}</span>
                          )}
                        </span>
                      </div>
                    )
